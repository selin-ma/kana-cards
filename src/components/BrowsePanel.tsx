...
export default function BrowsePanel({ cards }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return cards
    return cards.filter(
      (c) => c.roma.includes(q) || c.hira.includes(query) || c.kata.includes(query),
    )
  }, [cards, query])
      ...
      <div className='flex flex-col gap-5 w-full max-w-lg'>
        ...
        <div className='flex flex-col gap-5'>
          {[...byRow.entries()].map(([row, rowCards]) => (
            <div key={row}>
              <p className='text-xs mb-2' style={{ color: '#A8B4A8' }}>
                {row}
              </p>
                ...
                {rowCards.map((c) => (
                  ...
                  >
                    <div className='flex gap-1.5 items-baseline'>
                      <span className='text-base font-light' style={{ color: '#5A8870' }}>
                        {c.hira}
                      </span>
                      <span className='text-sm font-light' style={{ color: '#7A7A9A' }}>
                        {c.kata}
                      </span>
                    </div>
                    <span className='text-xs' style={{ color: '#B8C4B8' }}>
                      {c.roma}
                    </span>
                  </button>
                ))}
