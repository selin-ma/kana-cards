...
function MiniCard({
  ...
  card: KanaCard | undefined
  onClickCard: (c: KanaCard) => void
}) {
  ...
  return (
    ...
    >
      <div className='flex gap-1 items-baseline'>
        <span className='text-base font-light' style={{ color: hiColor[attempt.result] }}>
          {card?.hira ?? '?'}
        </span>
        <span className='text-sm font-light' style={{ color: kaColor[attempt.result] }}>
          {card?.kata ?? ''}
        </span>
      </div>
      <span style={{ color: romaColor[attempt.result], fontSize: '10px' }}>
        {attempt.card_id}
      </span>
    </button>
  )
...
export default function HistoryPanel({ records, allCards, onDelete }: Props) {
  ...
  return (
    <>
            ...
            return (
              <div key={r.id} className='flex items-center gap-2'>
                ...
                <button
                  ...
                  style={{
                    background: isChecked ? '#F0F6F0' : '#FEFCF8',
                    border: `1px solid ${isChecked ? '#C8DEC8' : '#E4EAE4'}`,
                  }}
                  onMouseEnter={(e) => {
                    if (!managing) e.currentTarget.style.background = '#F4F8F4'
                  }}
                  onMouseLeave={(e) => {
                    if (!managing)
                      e.currentTarget.style.background = isChecked ? '#F0F6F0' : '#FEFCF8'
                  }}
                >
                  ...
                  <div className='flex gap-3 text-xs'>
                    ...
                    {r.skipped > 0 && (
                      <span style={{ color: '#B8C4B8' }}>↷ {r.skipped}</span>
                    )}
        ...
        >
          <div
            ...
            style={{
              ...
              boxShadow: '0 8px 32px rgba(80,110,85,0.12)',
            }}
          >
            <p className='text-sm font-medium text-center' style={{ color: '#3A4A3C' }}>
              确认删除 {checked.size} 条记录？
            </p>
            <p className='text-xs text-center' style={{ color: '#B8C4B8' }}>
              此操作不可撤销
            </p>
        ...
        >
          ...
          >
            <div className='flex items-center justify-between'>
              <div>
                <p className='text-sm font-medium' style={{ color: '#3A4A3C' }}>
            ...
            {loadingDetail ? (
              ...
              <div className='flex flex-col gap-3'>
                {wrong.length > 0 && (
                  <div>
                    <p className='text-xs mb-1.5' style={{ color: '#C08878' }}>
                      没记住 ({wrong.length})
                    </p>
                    <div className='flex flex-wrap gap-1.5'>
                      {wrong.map((a) => (
                        <MiniCard
                          key={a.card_id}
                          attempt={a}
                          card={cardMap[a.card_id]}
                          onClickCard={setCardDetail}
                        />
                      ))}
                    </div>
                  </div>
                ...
                {correct.length > 0 && (
                  <div>
                    <p className='text-xs mb-1.5' style={{ color: '#7AAA7A' }}>
                      记住了 ({correct.length})
                    </p>
                    <div className='flex flex-wrap gap-1.5'>
                      {correct.map((a) => (
                ...
                {skipped.length > 0 && (
                  <div>
                    <p className='text-xs mb-1.5' style={{ color: '#B8C4B8' }}>
                      已跳过 ({skipped.length})
                    </p>
                    <div className='flex flex-wrap gap-1.5'>
                      {skipped.map((a) => (
      ...
      {cardDetail && (
        <KanaDetailModal card={cardDetail} onClose={() => setCardDetail(null)} />
      )}
    </>
  )
}
