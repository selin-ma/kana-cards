...
export default function VocabHistoryPanel({
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-sm">
      ...
      {loading && (
        <p className="text-sm" style={{ color: "#B8C4B8" }}>加载中…</p>
      )}
      {error && (
        <p className="text-sm" style={{ color: "#C08878" }}>{error}</p>
      )}
      ...
      <div className="flex flex-col gap-2">
        {records.map((r) => {
          const answered =
            r.again_count + r.hard_count + r.good_count + r.easy_count;
          const incomplete = answered + r.skipped_count < r.total;
          const knownRate =
            r.total > 0
              ? Math.round(((r.good_count + r.easy_count) / r.total) * 100)
              : 0;
